...
class TestServer(unittest.TestCase):
    ...
    def test_start_server(self, mock_thread, mock_exit, mock_signal, mock_socket, mock_ssl_context):
        """
        ...
        mock_socket_instance.accept.side_effect = [((mock_conn, mock_addr)), KeyboardInterrupt]
        
        # Call handle_new_connection directly within the threading patch to simplify test flow
        with patch('threading.Thread.start', side_effect=lambda: handle_new_connection(mock_conn, mock_addr, mock_context)):
            start_server()
